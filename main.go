...
func updatetemplateInformation(bitumdClient *rpcclient.Client, latestBlockHeader *wire.BlockHeader) {
	...
	stakeVersionResults, err := bitumdClient.GetStakeVersions(hash.String(),
	...
	intervalStakeVersions, err := bitumdClient.GetStakeVersions(hash.String(),
	...
	stakeVersionInfo, err := bitumdClient.GetStakeVersionInfo(numberOfIntervals)
	if err != nil {
		log.Println(err)
		return
	}
	...
	svis, err := AllStakeVersionIntervals(bitumdClient, height)
	if err != nil {
	...
	templateInformation.Agendas, err = agendasForVersions(bitumdClient, svis.MaxVoteVersion, height, svis)
	if err != nil {
...
func mainCore() int {
	...
	// Read in current bitumd cert
	var bitumdCerts []byte
	if !cfg.DisableTLS {
		bitumdCerts, err = ioutil.ReadFile(cfg.RPCCert)
		if err != nil {
			log.Printf("Failed to read bitumd cert file at %v: %v",
				cfg.RPCCert, err)
			return 1
		}
	}
	...
	log.Printf("Attempting to connect to bitumd RPC %s as user %s "+
		"using certificate %s", cfg.RPCHost, cfg.RPCUser, cfg.RPCCert)
	// Attempt to connect rpcclient and daemon
	bitumdClient, err := rpcclient.New(connCfgDaemon, &ntfnHandlersDaemon)
	if err != nil {
		log.Printf("Failed to start bitumd rpcclient: %v", err)
		return 1
	}
	defer func() {
		log.Printf("Disconnecting from bitumd.")
		bitumdClient.Disconnect()
	}()
	...
	hash, err := bitumdClient.GetBestBlockHash()
	if err != nil {
		log.Println(err)
		return 1
	}
	// Request the current block header
	latestBlockHeader, err := bitumdClient.GetBlockHeader(hash)
	if err != nil {
		log.Println(err)
		return 1
	}
	...
	updatetemplateInformation(bitumdClient, latestBlockHeader)
	...
	go func() {
		for {
			...
			case blkHdr := <-connectChan:
				...
				updatetemplateInformation(bitumdClient, &blkHdr)
			case <-quit:
				log.Println("Closing bitumvotingweb")
				wg.Done()
				return
			}
		}
