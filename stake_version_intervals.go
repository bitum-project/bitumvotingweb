...
// StakeVersionIntervals wraps a set of bitumjson.VersionIntervals
...
func (s *StakeVersionIntervals) GetStakeVersionUpgradeSVI(version uint32) (upgradeOccurred bool, upgradeSVI bitumjson.VersionInterval) {
	...
	return false, bitumjson.VersionInterval{}
}

// AllStakeVersionIntervals uses the bitumd client to create an ordered
// set of objects representing every Stake Version Interval up to the
// provided block height
func AllStakeVersionIntervals(bitumdClient *rpcclient.Client, height int64) (StakeVersionIntervals, error) {
	...
	// Get SVIs details from bitumd
	stakeVersionInfoResult, err := bitumdClient.GetStakeVersionInfo(totalSVIs)
	if err != nil {
