...
func (a *Agenda) DescriptionWithDCPURL() template.HTML {
	subst := `<a href="https://github.com/bitum-project/dcps/blob/master/dcp-${1}/dcp-${1}.mediawiki" target="_blank" rel="noopener noreferrer">${0}</a>`
...
// CountVotes uses the bitumd client to find all yes/no/abstain votes
// cast against this agenda. It will count the votes and store the
// totals inside the Agenda
func (a *Agenda) countVotes(bitumdClient *rpcclient.Client, votingStartHeight int64, votingEndHeight int64) error {
	// Find the last block hash of this voting period
	// Required to call GetStakeVersions
	lastBlockHash, err := bitumdClient.GetBlockHash(votingEndHeight)
	if err != nil {
		return err
	}

	// Retrieve all votes for this voting period
	stakeVersions, err := bitumdClient.GetStakeVersions(lastBlockHash.String(), int32(votingEndHeight-votingStartHeight))
	if err != nil {
		return err
	}

	// Collect all votes of the correct version
	var votes []bitumjson.VersionBits
...
func agendasFromJSON(getVoteInfo bitumjson.GetVoteInfoResult) []Agenda {
...
func agendasForVersions(bitumdClient *rpcclient.Client, maxVoteVersion uint32, currentHeight int64, svis StakeVersionIntervals) ([]Agenda, error) {
	var allAgendas []Agenda
	for version := uint32(0); version <= maxVoteVersion; version++ {
		// Retrieve Agendas for this voting period
		getVoteInfo, err := bitumdClient.GetVoteInfo(version)
		if err != nil {
		...
		for _, agenda := range agendas {
			...
			err = agenda.countVotes(bitumdClient, votingStartHeight, votingEndHeight)
			if err != nil {
				return nil, err
			}
		}
